...
```{r}
vm_train |> model(STL(price, robust = TRUE)) |> components() |> autoplot()
vm_train |> model(STL(log(price), robust = TRUE)) |> components() |> autoplot()
vm_train |> model(STL(box_cox(price, lambda), robust = TRUE)) |> components() |> autoplot()
```